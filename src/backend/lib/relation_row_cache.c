 ...
 *                GlobalEntry*[ROW_CACHE_HASH_BUCKETS],链元素是 GlobalEntry。
 *
...
/*
 * 桶数是编译期定值 ROW_CACHE_HASH_BUCKETS = 2^24,掩码 ROW_CACHE_BUCKET_MASK,
 * 定义与取值理由见 lib/relation_row_cache.h。这里不再有运行期桶数变量:
 * `hash & ROW_CACHE_BUCKET_MASK` 直接是常量掩码,连一次间接寻址都省掉。
 */
...
static GlobalEntry **RowCacheBuckets = NULL;	/* 长度 ROW_CACHE_HASH_BUCKETS */
...
/*
 * row_cache_size 是"0 或 >= ROW_CACHE_MIN_ACTIVE_SIZE_MB"的二段式取值:
 * 0 表示特性整体关闭,启用时则必须够大。连续区间表达不了,故在此把关。
 */
bool
check_row_cache_size(int *newval, void **extra, GucSource source)
{
	if (*newval != 0 && *newval < ROW_CACHE_MIN_ACTIVE_SIZE_MB)
	{
		GUC_check_errdetail("\"row_cache_size\" must be 0 (disables the row cache) or at least %d MB.",
							ROW_CACHE_MIN_ACTIVE_SIZE_MB);
		return false;
	}
	return true;
}

/* 段数:池大小(MB)/ 段大小(1MB)。关闭时为 0。 */
static inline int32
...
RowCacheShmemSize(void)
{
	...
	/*
	 * 关闭时一个字节都不划。注意桶头数组按 ROW_CACHE_HASH_BUCKETS 计算,
	 * 与 row_cache_size 无关(默认 8M 桶就是 64MB)——不在这里提前返回,
	 * 光靠段数为 0 省不掉它。
	 */
	if (!RowCacheIsEnabled())
		return 0;

	sz = MAXALIGN(sizeof(RowCacheControl));
	nsegs = (Size) RowCacheNSegments();
...
RowCacheShmemInit(void)
{
	...
	/*
	 * 关闭:不划共享内存、不设任何基址。RowCacheCtl 保持 NULL,模块内所有
	 * 对外入口据此自动短路,行为与未打本补丁的原生 PG 一致。
	 */
	if (!RowCacheIsEnabled())
		return;

	nsegs = RowCacheNSegments();
...
SegUnlinkEntriesSlow(int32 sid)
{
	...
	while (off < used)
	{
		...
		bucket = e->pkey_hash & ROW_CACHE_BUCKET_MASK;
...
SegUnlinkEntries(int32 sid)
{
	...
	while (off < used && cnt < nmax)
	{
		...
		items[cnt].entry = e;
		items[cnt].bucket = e->pkey_hash & ROW_CACHE_BUCKET_MASK;
