...
#
# 用法：
#   ./relation_row_cache_multiproc_bench.sh [NPROCS]
#
# 可选环境变量：
...
DO \$\$
DECLARE
    v_loops  int := ${LOOPS};    -- bash 展开在这里，赋给 PL/pgSQL 变量
    v_rounds int := ${ROUNDS};
...
BEGIN
    ...
    FOR i IN 1..v_rounds LOOP
        ...
        FROM generate_series(1, v_loops) AS g    -- 用变量 → SPI 编译成 $1
        CROSS JOIN LATERAL
        (
            SELECT b AS r
...
run_phase() {
  ...
  for i in $(seq 1 "$NPROCS"); do
    ...
    if [[ "$ms" != "NA" ]]; then
      ...
      if [[ -z "$min" ]] || (( $(echo "$ms < $min" | bc -l) )); then
        min="$ms"
      fi
      if [[ -z "$max" ]] || (( $(echo "$ms > $max" | bc -l) )); then
        max="$ms"
      fi
    fi
  done
  ...
  if [[ $count -gt 0 ]]; then
    ...
    local throughput
    throughput=$(echo "scale=1; $NPROCS * $ROUNDS * $LOOPS / ($wall_ms / 1000.0)" | bc)
...
if [[ -f "$OUT_DIR/summary.txt" ]]; then
  ...
  if [[ -n "$nocache_wall" && -n "$cache_wall" && "$cache_wall" != "0" ]]; then
    speedup_wall=$(echo "scale=2; $nocache_wall / $cache_wall" | bc)
    speedup_avg=$(echo "scale=2; $nocache_avg / $cache_avg" | bc)
